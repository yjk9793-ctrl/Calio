    ...
    <>
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .av { width:36px; height:36px; border-radius:18px; background:rgba(216,90,48,0.2); display:flex; align-items:center; justify-content:center; font-family:'Bricolage Grotesque',sans-serif; font-size:13px; font-weight:700; color:#D85A30; cursor:pointer; border:1px solid rgba(216,90,48,0.3); }
        ...
        .stat-val { font-family:'Bricolage Grotesque',sans-serif; font-size:30px; font-weight:800; color:#0F0E0D; line-height:1; }
        ...
        .ai-icon { width:36px; height:36px; border-radius:12px; background:#FAECE7; display:flex; align-items:center; justify-content:center; font-size:16px; flex-shrink:0; }
        ...
        .empty-ttl { font-family:'Bricolage Grotesque',sans-serif; font-size:18px; font-weight:800; color:#0F0E0D; margin-bottom:6px; }
        .empty-sub { font-size:14px; color:#aaa; font-weight:300; line-height:1.5; }

        /* FAB */
        .fab { position:fixed; bottom:calc(80px + env(safe-area-inset-bottom,0px)); right:20px; width:56px; height:56px; border-radius:28px; background:#D85A30; color:#fff; font-size:28px; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-family:'Bricolage Grotesque',sans-serif; line-height:1; z-index:10; box-shadow: 0 4px 16px rgba(216,90,48,0.35); }
        ...
        /* 진행 퍼센트 뱃지 */
        .pct-badge { background:rgba(216,90,48,0.15); border:1px solid rgba(216,90,48,0.3); border-radius:20px; padding:4px 12px; font-size:12px; font-weight:600; color:#D85A30; margin-top:6px; }
      `}</style>
        ...
        <div className="hero">
          ...
          <div className="ring-wrap">
            <svg width="200" height="200" viewBox="0 0 200 200">
              {/* 장식 원 */}
              <circle cx="100" cy="100" r={R1+16} fill="none" stroke="rgba(255,255,255,0.03)" strokeWidth="1"/>
              <circle cx="100" cy="100" r={R1+28} fill="none" stroke="rgba(255,255,255,0.02)" strokeWidth="1"/>
              ...
              {/* 끝점 dot */}
              {animPctIn > 2 && (() => {
                const angle = (animPctIn / 100) * 360 - 90
                const rad = angle * Math.PI / 180
                const x = 100 + R1 * Math.cos(rad)
                const y = 100 + R1 * Math.sin(rad)
                return <circle cx={x} cy={y} r="8" fill="#FF7A52"/>
              })()}
              ...
              <circle cx="100" cy="100" r={R2} fill="none" stroke="#1D9E75" strokeWidth="12"
                ...
                strokeLinecap="round" transform="rotate(-90 100 100)"/>
              {animPctOut > 2 && (() => {
                const angle = (animPctOut / 100) * 360 - 90
                const rad = angle * Math.PI / 180
                const x = 100 + R2 * Math.cos(rad)
                const y = 100 + R2 * Math.sin(rad)
                return <circle cx={x} cy={y} r="6" fill="#2DC98A"/>
              })()}

              {/* 중앙 */}
              <text x="100" y="86" textAnchor="middle"
                style={{ fontFamily:'Bricolage Grotesque,sans-serif', fontSize:11, fontWeight:600, fill:'rgba(255,255,255,0.4)', letterSpacing:'0.12em' }}>
                {remaining >= 0 ? 'REMAINING' : 'EXCEEDED'}
              </text>
              <text x="100" y="118" textAnchor="middle"
                style={{ fontFamily:'Bricolage Grotesque,sans-serif', fontSize:42, fontWeight:800, fill: remaining >= 0 ? '#fff' : '#FF6B6B' }}>
            ...
            <div className="ring-legend">
              <div className="leg"><div className="leg-dot" style={{ background:'#D85A30' }}/><span style={{ color:'rgba(255,255,255,0.6)' }}>섭취 {totalIn.toLocaleString()}</span></div>
              <div className="leg"><div className="leg-dot" style={{ background:'#1D9E75' }}/><span style={{ color:'rgba(255,255,255,0.6)' }}>소비 {totalOut.toLocaleString()}</span></div>
            </div>

            {/* 달성률 뱃지 */}
            <div className="pct-badge">
              목표 {goal.toLocaleString()} kcal · {Math.round(pctIn)}% 섭취
            </div>
          </div>
        </div>
