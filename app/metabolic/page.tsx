    ...
    <>
      <style>{`
        * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
        ...
        .hero { background:#0F0E0D; border-radius:0 0 32px 32px; padding:0 20px 28px; margin-bottom:20px; }
        ...
        .hero-title { font-family:'Bricolage Grotesque',sans-serif; font-size:32px; font-weight:800; color:#fff; line-height:1.15; letter-spacing:-0.5px; margin-bottom:8px; }
        .hero-sub { font-size:14px; color:rgba(255,255,255,0.4); font-weight:300; line-height:1.6; margin-bottom:20px; }
        ...
        .hero-main-num { display:flex; align-items:baseline; gap:8px; margin-bottom:6px; }
        .hero-main-val { font-family:'Bricolage Grotesque',sans-serif; font-size:48px; font-weight:800; color:#fff; line-height:1; }
        .hero-main-unit { font-size:16px; color:rgba(255,255,255,0.4); }
        .hero-main-note { font-size:12px; color:rgba(255,255,255,0.3); font-weight:300; display:flex; align-items:center; gap:6px; }
        ...
        .pattern-lock { position:absolute; inset:0; background:rgba(255,255,255,0.85); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; }
      ...
      <div className="mw">
        ...
        <div className="hero">
          ...
          <div className="hero-sub">
            공식 BMR이 아닌, 내 실제 기록으로 만든<br/>나만의 칼로리 공식이에요.
          </div>

          {/* 실제 유지 칼로리 */}
          <div className="hero-main">
            <div className="hero-main-lbl">내 실제 유지 칼로리</div>
            <div className="hero-main-num">
              {phase >= 2 ? (
                <>
                ...
                <span style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:22, color:'rgba(255,255,255,0.3)', fontWeight:800 }}>
                  30일 후 공개돼요
                </span>
              )}
            </div>
          ...
          <div className="hero-stats">
            <div className="hero-stat">
              <div className="hero-stat-v" style={{ color: phase >= 2 ? '#D85A30' : 'rgba(255,255,255,0.2)' }}>
                {phase >= 2 ? '-0.3' : '—'}
              </div>
              <div className="hero-stat-l">kg/월 평균</div>
            </div>
            <div className="hero-stat">
              <div className="hero-stat-v" style={{ color: phase >= 3 ? '#fff' : 'rgba(255,255,255,0.2)' }}>
                {phase >= 3 ? '3개' : '—'}
              </div>
              <div className="hero-stat-l">발견된 패턴</div>
            </div>
            <div className="hero-stat">
              <div className="hero-stat-v" style={{ color: phase >= 2 ? '#1D9E75' : 'rgba(255,255,255,0.2)' }}>
                {phase >= 2 ? '94%' : '—'}
              </div>
            ...
            <div key={i} className="pattern-item">
              ...
              {p.locked && (
                <div className="pattern-lock">
                  ...
                  <div className="pattern-lock-txt">
                    {i === 2 ? '90일 후 공개' : '60일 후 공개'}
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="timeline-card">
          ...
          {phases.map((p, i) => {
            ...
            const dotBg     = isDone ? '#1D9E75' : isCurrent ? '#D85A30' : '#F2F1EE'
            const dotColor  = isDone || isCurrent ? '#fff' : '#aaa'
            const lineColor = isDone ? '#1D9E75' : '#F2F1EE'
            return (
              <div key={p.num} className="tl-item">
                <div className="tl-left">
                  <div className="tl-dot" style={{ background: dotBg, color: dotColor }}>
                    {isDone ? '✓' : p.num}
                  </div>
                  {i < phases.length - 1 && (
                    <div className="tl-line" style={{ background: lineColor }}/>
                  )}
                </div>
