...
export default function MetabolicPage() {
  ...
  const handleSave = async () => {
    ...
    setWeightInput(''); setShowWeight(false); setLoading(true)
    ...
    <>
      <style>{`
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
        ...
        .w-header{padding:16px 18px;display:flex;justify-content:space-between;align-items:center;}
        .w-btn{background:#D85A30;border:none;border-radius:20px;padding:8px 16px;font-family:'Bricolage Grotesque',sans-serif;font-size:13px;font-weight:700;color:#fff;cursor:pointer;}
        .w-input-row{padding:12px 18px 16px;display:flex;gap:10px;align-items:center;border-top:0.5px solid rgba(0,0,0,0.05);}
        .w-input{flex:1;padding:12px 16px;border-radius:12px;border:none;background:#F7F5F2;font-family:'Bricolage Grotesque',sans-serif;font-size:20px;font-weight:700;color:#0F0E0D;outline:none;-webkit-appearance:none;}
        .w-save{background:#0F0E0D;border:none;border-radius:12px;padding:12px 18px;font-family:'Bricolage Grotesque',sans-serif;font-size:14px;font-weight:700;color:#fff;cursor:pointer;white-space:nowrap;}
        .w-latest{padding:0 18px 14px;display:flex;align-items:center;gap:8px;}
        .w-latest-val{font-family:'Bricolage Grotesque',sans-serif;font-size:24px;font-weight:800;color:#0F0E0D;}
        ...
        .prog-labels{display:flex;justify-content:space-between;font-size:11px;color:#aaa;}
      ...
      <div className="mw">
          ...
          <div className="h-main">
            ...
            <div className="h-main-row">
              {data?.estimatedMaintenance ? (
                <><span className="h-main-val">{data.estimatedMaintenance.toLocaleString()}</span><span className="h-main-unit">kcal/일</span></>
              ) : (
                <span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:17,color:'rgba(255,255,255,0.3)',fontWeight:700}}>체중 2회 이상 기록하면 공개돼요</span>
              )}
            </div>
            {data?.estimatedMaintenance && (
        ...
        <div className="disc">
          ...
          <div className="disc-txt"><strong>건강 보조 참고용 정보입니다.</strong> 기록 데이터 기반 통계적 추정이며, 의학적 진단이나 처방을 대체하지 않아요.</div>
        </div>
        ...
        <div className="card">
          <div className="w-header">
            <div>
              ...
              <div style={{fontSize:12,color:'#aaa',fontWeight:300,marginTop:3}}>
                {data?.latestWeight ? `최근 ${data.latestWeight}kg · ${data.weightCount}회 기록됨` : '체중 기록이 핵심이에요 — 대사율을 계산해요'}
              </div>
            </div>
            <button className="w-btn" onClick={() => setShowWeight(!showWeight)}>{showWeight ? '닫기' : '+ 기록'}</button>
          </div>

          {data?.latestWeight && !showWeight && (
            <div className="w-latest">
              <span className="w-latest-val">{data.latestWeight}</span>
              <span style={{fontSize:14,color:'#aaa'}}>kg</span>
              {data.firstWeight && data.firstWeight !== data.latestWeight && (
                <span style={{fontSize:13,fontWeight:600,marginLeft:'auto',color:data.latestWeight < data.firstWeight ? '#1D9E75':'#FF6B6B'}}>
          ...
          {showWeight && (
            <div className="w-input-row">
              <input className="w-input" type="number" step="0.1" placeholder="75.5" value={weightInput}
                onChange={e => setWeightInput(e.target.value)} inputMode="decimal"/>
              <span style={{fontSize:14,color:'#aaa'}}>kg</span>
              <button className="w-save" onClick={handleSave} disabled={saving}>{saving ? '저장 중...' : '저장'}</button>
            </div>
          )}
        </div>
        ...
        {(data?.weightCount ?? 0) === 0 && (
          <div className="nudge">
            ...
            <div>
              <div style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:15,fontWeight:800,color:'#fff',marginBottom:4}}>매일 아침 체중을 기록해보세요</div>
              <div style={{fontSize:12,color:'rgba(255,255,255,0.7)',fontWeight:300,lineHeight:1.5}}>기상 직후 체중 기록 → AI가 내 실제<br/>대사율을 정확히 계산해드려요.</div>
            </div>
          </div>
        )}
        ...
        <div className="card">
          <div className="card-inner">
            ...
            <div className="prog-labels"><span>시작</span><span style={{color:'#D85A30',fontWeight:600}}>{Math.round(progress)}%</span><span>완성</span></div>
            <div className="phase-badges">
              {[['기초 학습', phase>=2],['대사율 측정', phase>=3],['패턴 분석', phase>=4],['모델 완성', phase>=4]].map(([l,d],i) => (
                <div key={i} className="pb" style={{background:d?'#E1F5EE':'#F2F1EE',color:d?'#0F6E56':'#aaa'}}>{d?'✓ ':''}{l as string}</div>
              ))}
            </div>
          </div>
        </div>
                ...
                <div style={{flex:1}}>
                  ...
                  {!p.locked && p.confidence > 0 && (
                    <div className="pat-conf" style={{color:tc}}>
                      확신도 {p.confidence}%
                      <div className="conf-bg"><div className="conf-fill" style={{width:`${p.confidence}%`,background:fc}}/></div>
                    </div>
                  )}
                </div>
                {p.locked && <div className="lock"><span style={{fontSize:18}}>🔒</span><span style={{fontSize:12,color:'#aaa',fontWeight:300}}>더 많은 기록이 필요해요</span></div>}
              </div>
            )
          })}
        </div>
              ...
              return (
                <div key={p.num} className="tl-item">
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',flexShrink:0}}>
                    <div className="tl-dot" style={{background:isDone?'#1D9E75':isCur?'#D85A30':'#F2F1EE',color:isDone||isCur?'#fff':'#aaa'}}>{isDone?'✓':p.num}</div>
                    {i<3&&<div className="tl-line" style={{background:isDone?'#1D9E75':'#F2F1EE'}}/>}
                  </div>
                  <div style={{paddingTop:4}}>
                    <div style={{fontSize:11,fontWeight:600,marginBottom:2,color:isCur?'#D85A30':isDone?'#1D9E75':'#aaa'}}>{p.days}{isCur?' ← 지금 여기':''}</div>
